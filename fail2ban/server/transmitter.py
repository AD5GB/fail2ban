...
class Transmitter:
    ...
    def __commandSet(self, command, multiple=False):
        ...
        # Database
            ...
            if db is None:
                logSys.log(
                    logging.MSG, "dbmaxmatches setting was not in effect since no db yet")
                return None
            else:
            ...
            if db is None:
                logSys.log(
                    logging.MSG, "dbpurgeage setting was not in effect since no db yet")
                return None
            else:
        ...
        raise Exception(
            "Invalid command %r (no set action or not yet implemented)" % (command[1],))
